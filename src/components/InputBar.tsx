...
export const InputBar: React.FC<InputBarProps> = ({ platform, theme }) => {
  ...
  const containerStyle: React.CSSProperties = {
    ...
    gap: 16,
    backgroundColor: isWhatsApp ? '#f0f2f5' : '#f0f0f0',
  ...
  const inputContainerStyle: React.CSSProperties = {
    ...
    display: 'flex',
    alignItems: 'center',
    padding: isWhatsApp ? '16px 24px' : '10px 16px',
    borderRadius: 42,
    border: 'none',
    ...
    <div style={containerStyle}>
      {platform === 'messenger' && (
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
          <circle cx="12" cy="12" r="10" stroke="#0084ff" strokeWidth="2" fill="none" />
      ...
      {platform === 'imessage' ? (
        ...
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
          <path d="M2 21l21-9L2 3v7l15 2-15 2v7z" fill="#0084ff" />
        </svg>
      )}
    </div>
  );
