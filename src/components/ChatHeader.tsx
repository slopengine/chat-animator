...
export const ChatHeader: React.FC<ChatHeaderProps> = ({
  ...
  // Header row height: 44px in Figma
  ...
  // Back button - 44x44px hit area, positioned at left: 8px
  ...
  const renderBackButton = () => {
    if (platform === 'whatsapp') {
      return (
        ...
        >
          <svg width={10 * SCALE} height={18 * SCALE} viewBox="0 0 10 18" fill="none">
            <path
              d="M9 1L1 9L9 17"
              stroke="#000000"
              strokeWidth="2.5"
  ...
  // Icons should be same visual size - 22px as per code (SF Symbol size)
  const iconSize = 22 * SCALE;
  
  // Video icon - uses 29x28 viewBox, scale to fit 22px
  const renderVideoIcon = () => (
    <svg width={iconSize * 29/28} height={iconSize} viewBox="0 0 29 28" fill="none">
  ...
  // Phone icon - uses 21x21 viewBox, scale to fit 22px
  const renderPhoneIcon = () => (
    <svg width={iconSize} height={iconSize} viewBox="0 0 21 21" fill="none">
  ...
  return (
    <div style={headerStyle}>
      ...
      {/* Name and status - gap of 8px (single spacing) from avatar */}
      <div style={{ flex: 1, minWidth: 0, marginLeft: 8 * SCALE }}>
        <div
          style={{
            fontSize: 17 * SCALE, // 17px - iOS body-1-emphasized
            fontWeight: 600,
            color: '#000000',
            whiteSpace: 'nowrap',
            overflow: 'hidden',
            textOverflow: 'ellipsis',
            lineHeight: 1.29, // 22px line height / 17px
        ...
        {displaySubtitle && (
          <div 
            style={{ 
              fontSize: 13 * SCALE, // 13px - iOS body-3
              color: '#667781',
      ...
      {/* Action icons - 44x44px buttons, right: 8px */}
      {platform === 'whatsapp' && (
        <div style={{ display: 'flex', flexShrink: 0, alignItems: 'center', marginRight: 8 * SCALE }}>
          <div style={{ width: buttonSize, height: buttonSize, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {renderVideoIcon()}
          </div>
          <div style={{ width: buttonSize, height: buttonSize, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {renderPhoneIcon()}
